...
function formikReducer(state, msg) {
  switch (msg.type) {
    ...
    case 'SET_FIELD_VALUE':
      ...
      case 'SET_FIELD_LOV':
        return _extends({}, state, {
          values: setIn(state.lov, msg.payload.field, msg.payload.value)
        });

    case 'SET_FIELD_DISABLED':
      return _extends({}, state, {
        values: setIn(state.disabled, msg.payload.field, msg.payload.value)
      });

    case 'SET_FIELD_REQUIRED':
      return _extends({}, state, {
        values: setIn(state.required, msg.payload.field, msg.payload.value)
      });

    case 'SET_FIELD_HIDDEN':
      return _extends({}, state, {
        values: setIn(state.hidden, msg.payload.field, msg.payload.value)
      });
...
function useFormik(_ref) {
  ...
  var resetForm = React.useCallback(function (nextState) {
    ...
    var lov = nextState && nextState.lov ? nextState.lov : initialLov.current;
    var disabled = nextState && nextState.disabled ? nextState.disabled : initialDisabled.current;
    var required = nextState && nextState.required ? nextState.required : initialRequired.current;
    var hidden = nextState && nextState.hidden ? nextState.hidden : initialHidden.current;
