...
function formikReducer(state, msg) {
  switch (msg.type) {
    ...
    case 'SET_FIELD_LOV':
        return _extends({}, state, {
          lov: setIn(state.lov, msg.payload.field, msg.payload.value)
        });

    case 'SET_FIELD_DISABLED':
      return _extends({}, state, {
        disabled: setIn(state.disabled, msg.payload.field, msg.payload.value)
      });

    case 'SET_FIELD_REQUIRED':
      return _extends({}, state, {
        required: setIn(state.required, msg.payload.field, msg.payload.value)
      });

    case 'SET_FIELD_HIDDEN':
      return _extends({}, state, {
        hidden: setIn(state.hidden, msg.payload.field, msg.payload.value)
      });
    /* change ends here ) */
...
function useFormik(_ref) {
  ...
  var initialLov = useRef(props.initialLov || {});
  var initialDisabled = useRef(props.initialDisabled || {});
  var initialHidden = useRef(props.initialHidden || {});
  var initialRequired = useRef(props.initialRequired || {});
  ...
  var resetForm = useCallback(function (nextState) {
    ...
    var lov = nextState && nextState.lov ? nextState.lov : initialLov.current ? initialLov.current : props.initialLov || {};
    var disabled = nextState && nextState.disabled ? nextState.disabled : initialDisabled.current ? initialDisabled.current : props.initialDisabled || {};
    var hidden = nextState && nextState.hidden ? nextState.hidden : initialHidden.current ? initialHidden.current : props.initialHidden || {};
    var required = nextState && nextState.required ? nextState.required : initialRequired.current ? initialRequired.current : props.initialRequired || {};

    initialValues.current = values;
    initialErrors.current = errors;
    initialTouched.current = touched;
    initialStatus.current = status;
    initialLov.current = lov;
    initialDisabled.current = disabled;
    initialHidden.current = hidden;
    initialRequired.current = required;
  ...
  }, [props.initialErrors, props.initialStatus, props.initialTouched, props.initialLov, props.initialRequired, props.initialDisabled, props.initialHidden]);
  useEffect(function () {
    if (isMounted.current === true && !isEqual(initialValues.current, props.initialValues)) {
      if (enableReinitialize) {
        initialValues.current = props.initialValues;
        initialLov.current = props.initialLov;
        initialDisabled.current = props.initialDisabled;
        initialRequired.current = props.initialRequired;
        initialHidden.current = props.initialHidden;
        resetForm();
      }
  ...
  }, [enableReinitialize, props.initialValues, props.initialLov, props.initialDisabled, props.initialRequired, props.initialHidden, resetForm, validateOnMount, validateFormWithHighPriority]);
  useEffect(function () {
  ...
  var setLov = useEventCallback(function (values) {
    var resolvedValues = isFunction(values) ? values(state.values) : values;
    dispatch({
      type: 'SET_LOV',
      payload: resolvedValues
  ...
  var setDisabled = useEventCallback(function (values) {
    var resolvedValues = isFunction(values) ? values(state.values) : values;
    dispatch({
      type: 'SET_DISABLED',
      payload: resolvedValues
  ...
  var setRequired = useEventCallback(function (values) {
    var resolvedValues = isFunction(values) ? values(state.values) : values;
    dispatch({
      type: 'SET_REQUIRED',
      payload: resolvedValues
  ...
  var setHidden = useEventCallback(function (values) {
    var resolvedValues = isFunction(values) ? values(state.values) : values;
    dispatch({
      type: 'SET_HIDDEN',
      payload: resolvedValues
  ...
  var setFieldLov = useEventCallback(function (field, value) {
    dispatch({
      type: 'SET_FIELD_LOV',
      payload: {
        field: field,
  ...
  var setFieldDisabled = useEventCallback(function (field, value) {
    dispatch({
      type: 'SET_FIELD_DISABLED',
      payload: {
        field: field,
        value: value
      }
  ...
  var setFieldRequired = useEventCallback(function (field, value) {
    dispatch({
  ...
  var setFieldHidden = useEventCallback(function (field, value) {
    dispatch({
      type: 'SET_FIELD_HIDDEN',
      payload: {
        field: field,
        value: value
      }
  ...
  var getFieldMeta = useCallback(function (name) {
    return {
      ...
      initialLov: getIn(initialLov.current, name),
      initialDisabled: getIn(initialDisabled.current, name),
      initialRequired: getIn(initialRequired.current, name),
      initialHidden: getIn(initialHidden.current, name)
    };
  ...
  var getFieldHelpers = useCallback(function (name) {
    return {
      ...
      setLov: function setValue(value) {
        return setFieldLov(name, value);
      },
      setDisabled: function setValue(value) {
        return setFieldDisabled(name, value);
      },
      setRequired: function setValue(value) {
        return setFieldRequired(name, value);
      },
      setHidden: function setValue(value) {
        return setFieldHidden(name, value);
      },
  ...
  }, [setFieldValue, setFieldLov, setFieldDisabled, setFieldRequired, setFieldHidden, setFieldTouched, setFieldError]);
  var getFieldProps = useCallback(function (nameOrOptions) {
    var isAnObject = isObject(nameOrOptions);
    var name = isAnObject ? nameOrOptions.name : nameOrOptions;
    var valueState = getIn(state.values, name);
    var lovState = getIn(state.lov, name);
    var disabledState = getIn(state.disabled, name);
    var requiredState = getIn(state.required, name);
    var hiddenState = getIn(state.hidden, name);
    ...
    return field;
  }, [handleBlur, handleChange, state.values, state.lov, state.disabled, state.required, state.hidden]);
  ...
  var ctx = _extends({}, state, {
    ...
    initialLov: initialLov.current,
    initialDisabled: initialDisabled.current,
    initialRequired: initialRequired.current,
    initialHidden: initialHidden.current,
  ...
  function FieldArrayInner(props) {
    ...
    _this.updateArrayField = function (fn, alterTouched, alterErrors) {
      ...
      setFormikState(function (prevState) {
        ...
        var lov = setIn(prevState.lov, name, fn(getIn(prevState.lov, name)));
        var disabled = setIn(prevState.disabled, name, fn(getIn(prevState.disabled, name)));
        var required = setIn(prevState.required, name, fn(getIn(prevState.required, name)));
        var hidden = setIn(prevState.hidden, name, fn(getIn(prevState.hidden, name)));
    ...
    _this.addRow = function (addRowData) {
        let i = -1;
        let value = [
            // set values
            {
        ...
        return _this.updateArrayField(function (arrayLike) {
            ++i;
          return [].concat(copyArrayLike(arrayLike), [cloneDeep(value[i])]);
        }, false, false);
      };

      _this.handleAddRow = function (value) {
        return function () {
          return _this.addRow(value);
        };
      };
